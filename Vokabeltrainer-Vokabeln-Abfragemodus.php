...
if (isset($_SESSION['username'])) {
    ...
    if ($tableExists->num_rows == 0) {
        $createTableQuery = "CREATE TABLE `$tableName` (
            ...
            status ENUM('nicht gelernt', 'richtig', 'falsch') NOT NULL DEFAULT 'nicht gelernt',
            datum DATE NOT NULL
        )";
        $conn->query($createTableQuery);
    }

    $sqlMain = "SELECT deutsch, englisch FROM vokabeln ORDER BY id ASC";
    $resultMain = $conn->query($sqlMain);

    if ($resultMain->num_rows > 0) {
        while ($row = $resultMain->fetch_assoc()) {
            $vokabel = $conn->real_escape_string($row['deutsch']);
            $uebersetzung = $conn->real_escape_string($row['englisch']);

            $checkExistQuery = "SELECT id FROM `$tableName` WHERE deutsch='$vokabel' AND englisch='$uebersetzung' LIMIT 1";
            $checkExistResult = $conn->query($checkExistQuery);

            if ($checkExistResult->num_rows == 0) {
                $insertQuery = "INSERT INTO `$tableName` (deutsch, englisch, status, datum) 
                                VALUES ('$vokabel', '$uebersetzung', 'nicht gelernt', CURDATE())";
                $conn->query($insertQuery);
            }
        }
    }

    $fetchUserVocabs = "SELECT deutsch, englisch FROM `$tableName`";
    $resultUserVocabs = $conn->query($fetchUserVocabs);

    if ($resultUserVocabs->num_rows > 0) {
        while ($row = $resultUserVocabs->fetch_assoc()) {
            $vocabList[] = ["word" => $row["deutsch"], "translation" => $row["englisch"]];
        }
    }
} 
...
<body>
	...
	<script src="Sonstige-Scripte-u.-Vokabeln/TooltipFunktion.js"></script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            showInfoBox("Klick die Vokabelkarte an und prüfe, ob du sie schon auswendig kannst. Drücke dementsprechend falsch oder richtig.", 4000);
    ...
    ?>
        ...
        <div class="left-container">
            ...
            <br>
            <a href="Vokabeltrainer-Vokabeln-Lernen-Nicht-Gelernt.php" class="button">Lerne nur nicht gelernte Vokabeln</a>
    ...
    ?>
        <div class="square">
			<br>
            <center><h2>Übersicht Tests</h2></center>
            <br>
            <center><p><i>Bitte melde dich an, um Vokabeln zu lernen und deinen Fortschritt zu speichern.</i></p></center>
        </div>
    <?php
    }
    ?>
